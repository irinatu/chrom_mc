...
for file in inp_list:
    col = np.genfromtxt(file, delimiter=' ', skip_header  = 3, skip_footer = 3, usecols = (1, 3, 6)) # 1- iteracja, 3 - zaakcept krok, 5 - energy, 8 - gyration
    try:
        columns
...
plt.plot( columns[:, 1], "-", linewidth =0.5)
plt.axis([0,len(columns[:, 1]), columns[:, 1].min(), columns[:, 1].max()])
plt.title("Energy")
plt.show()
...
fig = plt.figure()
plt.plot( columns[:, 2], "r--", linewidth =0.5)
plt.axis([0,len(columns[:, 2]), 0, columns[:, 2].max()])
dlugosc = len(columns[:, 2])/5
start_mean = len(columns[:, 2]) - dlugosc
print np.mean(columns[start_mean:, 2])
plt.title("Gyration")
plt.show()
